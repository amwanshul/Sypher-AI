...
const getSmartUrl = (
  ...
  for (const [key, url] of Object.entries(USER_BOOKMARKS)) {
    if (lower.includes(key)) {
      const isEmailRoute = ['gmail', 'email', 'inbox'].includes(key)
      return { url, source: isEmailRoute ? 'Gmail' : 'Bookmark', skipScrape: isEmailRoute }
    }
  }
...
export default function registerWebAgent(ipcMain: IpcMain) {
  ipcMain.removeHandler('open-email-inbox')
  ipcMain.handle('open-email-inbox', async () => {
    try {
      await shell.openExternal('https://mail.google.com/mail/u/0/#inbox')
      return { success: true }
    } catch (error: any) {
      return { success: false, error: error?.message || 'Could not open Gmail inbox.' }
    }
  })
