// Uses the main-process ADB bridge so renderer tools never run phone commands directly.
const getGroqKey = async () => {
  const secureKeys = await window.electron.ipcRenderer.invoke('secure-get-keys').catch(() => null)
  return secureKeys?.groqKey || localStorage.getItem('iris_groq_api_key') || ''
}

export const executeAndroidPhoneCommand = async (command: string) => {
  try {
    const groqKey = await getGroqKey()
    const res = await window.electron.ipcRenderer.invoke('adb-execute-phone-action', {
      command,
      groqKey
    })

    window.dispatchEvent(
      new CustomEvent('phone-action-event', {
        detail: {
          command,
          intent: res?.intent,
          success: Boolean(res?.success),
          result: res?.speechText,
          data: res?.data,
          error: res?.error
        }
      })
    ...
    return res?.speechText || (res?.success ? 'Phone action completed.' : 'Phone action failed.')
  } catch (error: any) {
    return error?.message || 'Phone action failed.'
  }
}
