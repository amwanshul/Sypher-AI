...
import {
  ...
  openEmailInbox,
  performWebSearch
} from '@renderer/functions/apps-manager-api'
...
const DEFAULT_PERSONALITY = [
  '- **Creator:** Anshul Wankhede.',
  '- **Tone:** Clear, concise, technically capable English.',
  '- **Rule:** Never sound like a support bot. Stay direct, useful, and calm.',
  '- **Identity:** You are Sypher, the local neural OS companion.'
].join('\n')

const LANGUAGE_OVERRIDE = [
  '- Default to English only.',
  '- Do not use Hindi, Hinglish, Telugu, or any mixed-language response unless the latest user request explicitly asks for that language by name.',
  '- Do not imitate multilingual text from old transcript history, names, location, browser state, or prior model messages.',
  '- For tool failures, state the exact technical problem in plain English and give the next step.'
].join('\n')

const getMessageText = (message: any): string => {
  if (typeof message?.parts?.[0]?.text === 'string') return message.parts[0].text
  if (typeof message?.content === 'string') return message.content
  return ''
}

const isMixedLanguageText = (text: string): boolean =>
  NON_ENGLISH_SCRIPT_PATTERN.test(text) || ROMANIZED_MIXED_LANGUAGE_PATTERN.test(text)
...
const SYSTEM_NOTICE_ACK_PATTERN = /^\[System Notice\]\s*updated\.?$/i
...
const isSystemNoticeText = (text: string): boolean =>
  SYSTEM_NOTICE_PATTERN.test(text.trim()) || INTERNAL_CONTEXT_UPDATE_PATTERN.test(text)

const isPhoneCommandText = (text: string): boolean => PHONE_COMMAND_PATTERN.test(text.trim())

const sanitizePromptHistory = (history: any[]) => {
  ...
  return history
    ...
    .filter(
      (message) =>
        message.text &&
        !isSystemNoticeText(message.text) &&
        !isMixedLanguageText(message.text) &&
        !(message.role === 'model' && isLowValueModelFiller(message.text))
    )
    .slice(-6)
}
  ...
  async connect(): Promise<void> {
    ...
    const activePersonality =
      storedPersonality && storedPersonality.trim() !== ''
        ? `${storedPersonality.trim()}\n${LANGUAGE_OVERRIDE}`
        : `${DEFAULT_PERSONALITY}\n${LANGUAGE_OVERRIDE}`

    const SYPHER_SYSTEM_INSTRUCTION = `
...
## 🎯 TOOL PROTOCOLS
- **control_android_phone:** Use this for every phone/mobile-specific command: sending WhatsApp on the phone, taking phone screenshots, opening phone apps, Spotify/media control, and calls.
- **send_whatsapp:** Use this only for desktop WhatsApp automation when the user does not mention phone/mobile.
...
# 🧠 MEMORY (Last Context)
${JSON.stringify(sanitizePromptHistory(history))}
---
`

    const finalSystemInstruction = SYPHER_SYSTEM_INSTRUCTION + contextPrompt
          ...
          tools: [
            {
              functionDeclarations: [
                ...
                },
                {
                  name: 'open_file',
                  description:
                    'Open a file in its default system application. Use open_folder for folders/directories.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      file_path: { type: 'STRING', description: 'The absolute path to the file.' }
                ...
                },
                {
                  name: 'open_folder',
                  description:
                    'Open a folder/directory in the default system file manager. Use this for requests like "open projects folder", "open games folder", "open downloads", or "show documents folder". The folder_path can be a full path or a simple folder name.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      folder_path: {
                        type: 'STRING',
                        description: 'Folder path or folder name, e.g. "D:\\Projects", "projects", "downloads", "games".'
                      }
                    },
                    required: ['folder_path']
                  }
                ...
                },
                {
                  name: 'google_search',
                  description:
                    "ACTION: Opens a web browser tab. Use this ONLY when the user explicitly says 'open google', 'search for X in the browser', or wants a quick link opened. For Gmail/email inbox requests, use open_email. DO NOT use this for deep research, generating reports, or learning new data.",
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                ...
                },
                {
                  name: 'send_whatsapp',
                  description:
                    'Send a WhatsApp message through desktop WhatsApp automation. Do not use this when the user says phone/mobile; use control_android_phone instead. If the user wants to send a file, provide the file_path.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                ...
                },
                {
                  name: 'google_search',
                  description:
                    'Search Google or open a public website. For Gmail/email inbox requests, use open_email.',
                  parameters: {
                    type: 'OBJECT',
                ...
                },
                {
                  name: 'open_project',
                  description:
                    'Open a folder in VS Code. Use only when the user explicitly says VS Code, code editor, coding project, or open project in code. For normal folder opening, use open_folder.',
                  parameters: {
                    type: 'OBJECT',
                    properties: { folder_path: { type: 'STRING' } },
                    required: ['folder_path']
                ...
                },
                {
                  name: 'open_email',
                  description:
                    'Open the user\'s Gmail inbox in the browser. Use this for requests like "open my email", "open Gmail", "show my inbox", or "go to my email". This does not read email content and does not require Gmail OAuth.',
                  parameters: { type: 'OBJECT', properties: {}, required: [] }
                },
                {
                  name: 'read_emails',
                  description:
                    'Read the latest Primary-tab messages from the user\'s Gmail inbox, matching Gmail.com Primary inbox as closely as the Gmail API allows. Use this only when the user asks to read/check/summarize emails, e.g. "check my emails" or "do I have any new emails?". Do not use this for "open my email" or "open Gmail"; use open_email instead.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      max_results: {
                ...
                {
                  name: 'control_android_phone',
                  description:
                    'Parse and execute a natural Android phone command through the local ADB bridge. Use this for: "send a WhatsApp to mom, I will be late", "text Aryan", "take a screenshot of my phone", "open Spotify on my phone", "play lo-fi on Spotify", "pause music", "next song", or "call mom".',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      command: {
                        type: 'STRING',
                        description:
                          'The full original user request that targets the Android phone.'
                      }
                    },
                    required: ['command']
                  }
                ...
                },
                {
                  ...
                  description:
                    'Instantly locks the SYPHER OS system, disconnects the AI, and returns the user to the secure biometric lock screen. Use this strictly when the user says "Lock the system", "Lock down", or "Activate Sentry Mode".',
                  parameters: {
                    type: 'OBJECT',
                    properties: {}
                  }
    ...
    this.socket.onmessage = async (event) => {
      try {
        ...
        if (serverContent?.interrupted) {
          ...
          if (this.contextUpdateSuppressTimer) {
            clearTimeout(this.contextUpdateSuppressTimer)
          ...
          await Promise.all(
            functionCalls.map(async (call: any) => {
              ...
              if (call.name === 'index_directory') {
                ...
                result = await openFolder(call.args.folder_path)
              ...
              } else if (call.name === 'open_email') {
                result = await openEmailInbox()
              ...
              } else if (call.name === 'control_android_phone') {
                result = await executeAndroidPhoneCommand(call.args.command)
        ...
        if (serverContent) {
          if (serverContent.modelTurn?.parts) {
            serverContent.modelTurn.parts.forEach((part: any) => {
              if (part.inlineData && !this.suppressContextUpdateResponse) {
          ...
          if (serverContent.interrupted) {
            if (this.suppressContextUpdateResponse) {
              this.aiResponseBuffer = ''
              this.userInputBuffer = ''
              this.suppressContextUpdateResponse = false
              if (this.contextUpdateSuppressTimer) {
            ...
            if (this.userInputBuffer.trim()) {
              const transcript = this.userInputBuffer.trim()
              if (!isMixedLanguageText(transcript) && !isSystemNoticeText(transcript)) {
          ...
          if (serverContent.turnComplete) {
            ...
            if (this.aiResponseBuffer.trim()) {
              const transcript = this.aiResponseBuffer.trim()
              if (
                !isMixedLanguageText(transcript) &&
                !isLowValueModelFiller(transcript) &&
                !isSystemNoticeText(transcript)
              ) {
                await saveMessage('iris', transcript)
              }
              this.aiResponseBuffer = ''
            }
          }
  ...
  async sendTextPrompt(prompt: string): Promise<{ ok: boolean; error?: string }> {
    ...
    if (isPhoneCommandText(text)) {
      await saveMessage('user', text)
      const result = await executeAndroidPhoneCommand(text)
      if (
        result &&
        !isMixedLanguageText(result) &&
        !isLowValueModelFiller(result) &&
        !isSystemNoticeText(result)
      ) {
        await saveMessage('iris', result)
      }
      return { ok: true }
    }

    if (!this.socket || this.socket.readyState !== WebSocket.OPEN || !this.isConnected) {
      return { ok: false, error: 'Start Sypher before sending a typed message.' }
    }
...
export const sypherService = new GeminiLiveService()
