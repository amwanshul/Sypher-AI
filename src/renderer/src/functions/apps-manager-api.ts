...
export const openEmailInbox = async () => {
  try {
    const result: any = await window.electron.ipcRenderer.invoke('open-email-inbox')
    if (result?.success) return 'Opening Gmail inbox.'
    return `Error: ${result?.error || 'Could not open Gmail inbox.'}`
  } catch (err) {
    return 'System Error: Could not open Gmail inbox.'
  }
}
