...
export const revealFile = async (filePath: string) => {
  try {
    const result = await window.electron.ipcRenderer.invoke('file:reveal', filePath)
    if (result.success) return 'File revealed successfully.'
    return `Error revealing file: ${result.error}`
  } catch (err) {
    return `System Error: ${err}`
  }
}
...
export const openFolder = async (folderPath: string) => {
  try {
    const result = await window.electron.ipcRenderer.invoke('folder:open', folderPath)
    if (result.success) return `Opened ${result.path || folderPath}.`
    return `Error opening folder: ${result.error}`
  } catch (err) {
    return `System Error: ${err}`
  }
}
