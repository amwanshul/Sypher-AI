...
const MiniOverlay = ({
  ...
  useEffect(() => {
      if (isSystemActive && sypherService.analyser) {
        analyzerRef.current = sypherService.analyser
        dataArrayRef.current = new Uint8Array(sypherService.analyser.frequencyBinCount)
      const checkAudio = () => {
