// Uses the existing Electron ADB bridge to render phone state and action feed without a separate server.
...
const PhoneView = ({ glassPanel }: { glassPanel?: string }) => {
  ...
  const [phoneFeed, setPhoneFeed] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    window.addEventListener('phone-link-history-cleared', handleHistoryCleared)
    return () => window.removeEventListener('phone-link-history-cleared', handleHistoryCleared)
  }, [])

  useEffect(() => {
    const removeListener = window.electron.ipcRenderer.on(
      'phone-dashboard-event',
      (_event: any, payload: any) => {
        if (!payload) return

        setPhoneFeed((current) => [payload, ...current].slice(0, 8))

        if (payload.speak) {
          window.dispatchEvent(
            new CustomEvent('ai-force-speak', {
              detail: payload.speak
            })
          )
        }
      }
  ...
  const executeMediaFeedCommand = async (mediaCmd: 'play' | 'pause' | 'next', app?: string) => {
    try {
      await window.electron.ipcRenderer.invoke('adb-execute-phone-action', {
        action: {
    ...
    <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-10 p-10 animate-in fade-in duration-500 bg-[#0a0a0a] min-h-screen overflow-y-auto">
      ...
      <div className="w-1/4 flex flex-col h-162.5 relative">
        <div className="bg-[#111] border border-white/5 rounded-2xl p-6 flex flex-col h-full shadow-lg">
          ...
          {phoneFeed.length > 0 && (
            <div className="mb-6 max-h-48 overflow-y-auto rounded-xl border border-white/5 bg-black/35 p-3 scrollbar-small">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-[9px] font-bold tracking-widest text-zinc-500">
                  ACTION FEED
                </span>
                <span className="text-[8px] font-mono text-purple-400/70">ADB LIVE</span>
              </div>
              <div className="space-y-2">
                {phoneFeed.map((item, index) => (
                  <div
                    key={`${item.timestamp || index}-${index}`}
                    className="rounded-lg border border-white/5 bg-zinc-950/70 p-3"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className="truncate text-[10px] font-bold uppercase tracking-widest text-zinc-300">
                        {String(item.action || item.type || 'phone action').replace(/_/g, ' ')}
                      </span>
                    ...
                    {(item.contact || item.app || item.preview || item.query || item.nowPlaying) && (
                      <p className="mt-2 line-clamp-2 text-[10px] font-mono leading-relaxed text-zinc-500">
                        {[item.contact, item.app, item.preview, item.query, item.nowPlaying]
                          .filter(Boolean)
                          .join(' / ')}
                      </p>
                    ...
                    {item.image && (
                      <img
                        ...
                        className="mt-3 max-h-28 w-full rounded-md border border-white/5 object-contain bg-black"
                      />
                    )}
                    {item.action === 'media_control' && (
                      <div className="mt-3 flex gap-2">
                        {(['play', 'pause', 'next'] as const).map((mediaCmd) => (
                          <button
                            key={mediaCmd}
                            onClick={() => executeMediaFeedCommand(mediaCmd, item.app)}
                            className="rounded-md border border-purple-500/20 bg-purple-500/10 px-2 py-1 text-[8px] font-bold uppercase tracking-widest text-purple-300 hover:bg-purple-500 hover:text-black"
                          >
                            {mediaCmd}
                          </button>
                        ))}
