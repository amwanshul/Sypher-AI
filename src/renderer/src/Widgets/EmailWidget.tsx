...
export default function EmailWidget() {
  ...
  useEffect(() => {
    const handleEvent = (event: any) => {
      const { emails, error } = event.detail

      setErrorMessage(typeof error === 'string' && error.trim() ? error : null)
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-mono mt-1 uppercase">
                {selectedEmail
                  ...
                  : `INTERCEPTED BY SYPHER // ${emails.length} MESSAGES`}
              </p>
            </div>
          </div>
          <button
                ...
                {errorMessage ? (
                  <div className="h-full flex flex-col items-center justify-center text-center gap-4 px-8">
                    ...
                    <div className="max-w-2xl">
                      <p className="text-xs tracking-widest text-red-300 font-mono mb-3">
                        GMAIL ACCESS ERROR
                      </p>
